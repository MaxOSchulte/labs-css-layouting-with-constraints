...
export class GroupComponent extends CellDirective implements AfterContentInit {
  ...
  @ContentChildren(CellSelectorDirective) filledCells?: QueryList<CellSelectorDirective>;

  ngAfterContentInit() {
    this.filledCells?.forEach(cell => console.log(cell.height, cell.cellHeightSum$$?.value))
    this.filledCells?.changes.pipe(
      startWith(this.filledCells),
      filter((cells: QueryList<CellSelectorDirective>) => cells.some(cell => cell instanceof SubGroupComponent))
    ).subscribe((filledCells: QueryList<CellSelectorDirective>) => {
      const heights = filledCells.map(({
        height,
        cellHeightSum$$,
      }) => cellHeightSum$$?.value ?? Number.parseInt((height ?? 1) + '', 10));

      this.cellHeightSum = heights.reduce((sum, height) => sum + height, 0)
